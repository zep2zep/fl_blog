...
def homepage():
    ...
    posts = Post.query.order_by(
        Post.created_at.desc()).paginate(page_number, 5, True)
...
def tempe():
    ...
    if request.method == 'POST':
        ...
        weather = {
            ...
            'sunrise': datetime.datetime.fromtimestamp(json_object['sys']['sunrise']).strftime('%H:%M:%S'),
            'sunset': datetime.datetime.fromtimestamp(json_object['sys']['sunset']).strftime('%H:%M:%S'),
